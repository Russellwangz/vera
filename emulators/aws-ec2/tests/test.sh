...
awscli ec2 create-ipam --description "Example description" --operating-regions "RegionName=us-east-2" "RegionName=us-west-1" --tag-specifications ResourceType=ipam,Tags=[{Key=Name,Value=ExampleIPAM}]
awscli ec2 create-key-pair --key-name MyKeyPair --query "KeyMaterial" --output text > /dev/null
...
awscli ec2 get-instance-types-from-instance-requirements --region us-east-1 --generate-cli-skeleton input > /dev/null
awscli ec2 get-serial-console-access-status
awscli ec2 get-snapshot-block-public-access-state
awscli ec2 get-spot-placement-scores --region us-east-1 --generate-cli-skeleton input > /dev/null
